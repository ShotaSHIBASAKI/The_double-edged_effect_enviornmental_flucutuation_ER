...
f=ggplot(df_new[df_new$Condition != 'Control', ], aes(x=Condition, y=Mu, color=Condition))+
  ...
  scale_x_discrete(labels=c(TeX("$\\sigma=0.000$"),
                            TeX("$\\sigma=0.025$"), TeX("$\\sigma=0.000$")))+
...
First we plot the probability of evolutionary rescue
```{r Plot ER simulations}
library(ggplot2)
library(patchwork)
library(nclbayes)
library(latex2exp)
...
g1+g2+g3+g4+g5+g6+g7+g8+plot_layout(ncol=2, nrow=4)+plot_annotation(tag_levels = "A",) & theme(plot.tag = element_text(size = 24))
ggsave('ER_simulations.png',width=15, height=18)
```

```{r distibution of traits}
library(ggplot2)
library(patchwork)
library(quantreg)
df=read.csv("ER_model_parameter_sweep2.csv")
...
g1=ggplot(df_rescue2, aes(x=as.factor(SD), y=Mean))+geom_violin() +
...
g2=ggplot(df_rescue2, aes(x=as.factor(SD), y=Variance))+geom_violin() +
  stat_summary(fun.y=median, geom="point", size=2, color="black")+
...
g1+g2+g3+g4+g5+g6+g7+g8+plot_layout(ncol=2, nrow=4)+plot_annotation(tag_levels = "A",) & theme(plot.tag = element_text(size = 24))
ggsave('ER_simulations2.png',width=15, height=18)
