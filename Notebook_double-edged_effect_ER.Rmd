...
stat1=data.frame(group1=group1,
                   ...
                   p.adj.signif=signif,
                   EffectSize=effsize_list)
#------------Is growth rate Mu  positive?--------------
p_list=c()
effsize_list=c()
W_list=c()
group=c("Linear", "SD025", "SD100")
for(i in 1:length(group)){
  W=wilcox_test(Mu~ 1, 
             data=df_new[df_new$Condition==group[i] ,], mu=0, "greater")
  w_effect=wilcox_effsize(Mu~ 1, 
             data=df_new[df_new$Condition==group[i] ,], mu=0, "greater")$effsize
  W_list=append(W_list, W$statistic)
  p_list=append(p_list, W$p)
  effsize_list=append(effsize_list, w_effect)
}
...
stat_positive=data.frame(group=group,
                   W=W_list,
                   #P.val=p_list,
                   P.FDR=p.corr,
                   p.adj.signif=signif,
